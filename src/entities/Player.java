...
public class Player extends Entity {
	...
	private int[] milestones = { 250, 500, 1000, 2500, 10000 }; // Define score milestones
	...
	public void update() {
		...
		if (Score > topScore) {
			getfile = false;
			writeTopScoreToFile("topscore.txt");
		}
	...
	public void resetAll() {
		...
		animalX = 900;
		animalX2 = 1000;
		if (animalHitbox1 != null) {
			animalHitbox1.x = animalX;
		}
		if (animalHitbox2 != null) {
			animalHitbox2.x = animalX2;
		}
	...
	private void reloadAnimation() {
		...
		System.out.println("Animations reloaded for Char_option: " + seleted); // Debugging
	}

	public void writeTopScoreToFile(String filename) {
		File file = resolveScoreFile(filename, true);
	...
	public int readTopScoreFromFile(String filename) {
		int topScore = 0;
		File file = resolveScoreFile(filename, false);
	...
	public void displayTopScore(Graphics g, int x, int y) {
		topScore = readTopScoreFromFile("topscore.txt");
	...
	private File resolveScoreFile(String filename, boolean preferWritable) {
		File[] candidates = { new File("rsc", filename), new File("platform2D/rsc", filename), new File(filename) };
